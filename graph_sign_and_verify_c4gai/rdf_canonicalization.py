...
class IdentifierIssuer:
    ...
    def __init__(self) -> None:
        self.ids: dict[rdflib.BNode, str] = {}
        self.counter = itertools.count()

    def get_id(self, blank_node: rdflib.BNode) -> str:
...
class RdfCanonicalization:
    ...
    def __init__(self, graph: Graph, max_run_time: int | None = None) -> None:
        ...
        self.non_normalized: set[rdflib.BNode] = set()
    ...
    def collect_blank_nodes(self) -> None:
    ...
    def issue_canonical_ids(self) -> None:
    ...
    def issue_n_degree_ids(self) -> None:
    ...
    def hash_n_degree(self, blank_id: rdflib.BNode) -> str:
        """Compute n-degree hash for a blank node by considering its connected triples recursively.

        :param blank_id: the blank node to hash.
        :return: the computed hash as a string.
        """
        temp_issuer = IdentifierIssuer()
        stack: list[tuple[rdflib.term.Node, tuple | None]] = [(blank_id, None)]
        visited = set()
        ...
        while stack:
            ...
            stack.extend(
                (triple[2], triple) for triple in self.graph.triples((current_node, None, None))
            )
            stack.extend(
                (triple[0], triple) for triple in self.graph.triples((None, None, current_node))
            )
    ...
    def compute_canonical_hash(
        self, permutation: tuple | list, temp_issuer: IdentifierIssuer
    ) -> str:
        ...
        for blank_id in permutation:
            ...
            for quad in quads:
                serialized_components.extend(
                    self.serialize_quad(quad, blank_id).replace("_:a", temp_id)
                )

        serialized_data = "".join(sorted(serialized_components)).encode("utf-8")
        return hashlib.sha256(serialized_data).hexdigest()

    def hash_first_degree(self, blank_id: rdflib.BNode) -> str:
    ...
    def serialize_quad(self, quad: tuple, blank_id: rdflib.BNode) -> str:
    ...
    def serialize_normalized_graph(self) -> str:
        ...
        for s, p, o in self.graph:
            s_replaced = self.replace_blank_with_canonical(s)
            o_replaced = self.replace_blank_with_canonical(o)
            normalized_graph.add((s_replaced, p, o_replaced))
        return normalized_graph.serialize(format="nt")  # Return as N-Triples for simplicity

    def replace_blank_with_canonical(self, term: rdflib.term.Node) -> rdflib.term.Node:
